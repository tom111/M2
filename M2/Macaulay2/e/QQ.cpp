...
QQ::QQ(const Monoid *D) : Ring(0,0,0,this /* Visual C WARNING */,
			     Monoid::get_trivial_monoid(),D)
{
  mpq_stash = new stash("QQ", sizeof(mpq_t));
  zero_elem = new_elem(); // this sets the element to 0.
}
...
{
  M2_Rational result = (M2_Rational) mpq_stash->new_elem();
  mpq_init(result);
  return result;
}
...
{
  mpq_sub(MPQ_VAL(f), zero_elem, MPQ_VAL(f));
}
...
{
  M2_Rational result = QQ::new_elem();
  mpq_sub(result, zero_elem, MPQ_VAL(f));
  return MPQ_RINGELEM(result);
}
