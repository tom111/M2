				  ...
				  const Vector_array *V)
{
  ...
  for (unsigned int i=0; i<V->len; i++)
    result->append(target->translate(V->array[i]->free_of(), 
				     V->array[i]->get_value()));
  return result;
}
				  ...
				  const Vector_array *V)
{
  ...
  if (R->degree_monoid()->n_vars() != deg->len)
    {
  ...
  for (unsigned int i=0; i<V->len; i++)
    (*result)[i] = (target->translate(V->array[i]->free_of(), 
				   V->array[i]->get_value()));
  return result;
}
				  ...
				  const Matrix *M)
{
  ...
  for (int i=0; i<source->rank(); i++)
    (*result)[i] = target->translate(M->rows(),
				     (*M)[i]);
  return result;
}
  ...
  // which occur in 'this'.  These monomials are sorted into increasing degree order.
{
  ...
  ring_elem one = get_ring()->Ncoeffs()->from_int(1);
  ...
  for (int c=0; c<n_cols(); c++)
      ...
      for ( ; v != 0; v = v->next)
	{
	  ...
	  rows()->to_exponents(v->monom, v->comp, exp);
	  for (unsigned int i=0; i<vars->len; i++)
  ...
  for (int i=0; monoms[i] != 0; i += 2)
	...
	exp[vars->array[j]] = exp1[j];
      ...
      result->append(rows()->term(0,one,mon));
    }
...
{
  if (f == NULL) return 0;
  const Monoid *M = F->get_ring()->Nmonoms();
  if (M == NULL) return 0;
  ...
  for (vec g = f ; g != 0; g = g->next)
    {
      F->to_exponents(g->monom, g->comp, exp);
      for (unsigned int i=0; i<vars->len; i++)
	{
	  int v = vars->array[i];
	  scratch_exp[i] = exp[v];
      ...
      if (val > 0)
	{
	  vec t = F->copy_term(g);
	  t->comp = val-1;
	  F->from_exponents(exp, t->comp, t->monom);
	  t->next = result;
	  result = t;
	}
    }
