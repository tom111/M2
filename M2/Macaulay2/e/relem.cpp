...
RingElement::RingElement(const Ring *R, const RingElement *a, const Monomial *m) : 
  immutable_object(0), R(R)
{
  intarray ma;
  int *mon = ma.alloc(R->n_vars());
  R->Nmonoms()->from_varpower(m->ints(), mon);
  val = R->term(R->Ncoeffs()->copy(a->get_value()), mon);
}
