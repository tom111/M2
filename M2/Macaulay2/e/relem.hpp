...
public:
  // Make these constructors all private
  // and have routines RingElement::make(...);
  ...
  RingElement(const Ring *R, const RingElement *a, const Monomial *m);
...
inline RingElement::RingElement(const Ring *R) : 
  immutable_object(0), R(R), val(R->from_int(0))
{  
}

inline RingElement::RingElement(const Ring *R, int n) : 
  immutable_object(0), R(R), val(R->from_int(n))
{
}
...
inline RingElement::RingElement(const Ring *R, int v, int e) : 
  immutable_object(0), R(R), val(R->var(v,e))
{
}
