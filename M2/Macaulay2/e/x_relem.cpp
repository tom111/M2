...
const Ring *IM2_Ring_RR(double precision, Monoid *deg_monoid)
{
...
const RingOrNull *IM2_Ring_weyl_algebra(const Ring *K, 
					...
					int homog_var)
{
  return WeylAlgebra::create(K,M,diff_vars, comm_vars, homog_var);
}
...
const RingOrNull *IM2_Ring_schur(const Ring *K, const Monoid *M)
{
  return new SchurRing(K,M);
}
