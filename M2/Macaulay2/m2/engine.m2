...
ZZ.RawRing = rawZZ()
QQ.RawRing = rawFractionRing rawZZ()
...
RawRing _ ZZ := (R,n) -> rawRingVar(R,n,1);
...
rawToRational = x -> (
     if rawRing x === QQ.RawRing
     then rawToInteger rawNumerator x / rawToInteger rawDenominator x
     else error "rawToRational not implemented yet"
     )
new ZZ from RawRingElement := (ZZ,n) -> rawToInteger n
new QQ from RawRingElement := (QQ,n) -> rawToRational n
