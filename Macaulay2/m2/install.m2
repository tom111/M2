...
names include these:
     ...
     @PACKAGE_TARNAME@-@PACKAGE_VERSION@-@ARCH@-@OS@-@ISSUE@.tar.gz

If there is a later version than @VERSION@, use that instead. 
...
as "@ARCH@-@OS@-@ISSUE@" in the name contain the architecture-dependent files,
...
in /foo/bar, as follows.
     ...
     tar zxfp @PACKAGE_TARNAME@-@PACKAGE_VERSION@-@ARCH@-@OS@-@ISSUE@.tar.gz
...
/foo/bar/Macaulay2-@VERSION@ has been created.  The subdirectories and files
...
-------------------------------------------------
   ...
   unless you add /foo/bar/Macaulay2-@VERSION@/lib to the search path.  Under
