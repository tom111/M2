...
K12 = ideal(x_2^3-x_1^2*x_3,x_1^3*x_4-x_1^2*x_2*x_3,x_1^4*x_5-x_1^2*x_2^2*x_3) -- the old answer from 1.2 and before 10/14/2009, wrong
...
assert isSubset(J,K12)
assert isSubset(x_1*K12, J)
...
assert isSubset(K13,K) -- we should keep doing at least as well (this is the first assertion failure for 1.2)
assert( K == K13 )     -- if we do better, get notified (this works for 1.3 but not for 1.2)
